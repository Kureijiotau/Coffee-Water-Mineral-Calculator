...
Add a long **Live final readings** strip beside the salt table on wide screens. The salt table remains the main editing surface on the left, while the readings strip runs vertically down the right side of the salt table. On smaller screens, the strip stacks above the salt table. It uses the already-computed `result.finalIons` and derived values, so it updates on every existing Mixer calculation change:
...
The strip displays all modeled active ions plus modeled TDS as vertically distributed readings on desktop, creating a continuous companion surface alongside the full salt table. The mobile layout preserves the same values in a compact two-column card without hiding readings or changing the calculation.
...
Create a small presentational component for the side readings strip rather than duplicating ion formatting in the page body. It receives:
...
The side strip should remain within the Mixer document flow, stretch alongside the salt table on wide screens, and use a restrained background so it reads as a continuous companion surface without covering controls. The full output panel remains available near the top for users who want the detailed result view.
...
- Side readings remain keyboard-readable and are presented above the salt table on narrow screens.
...
- The live readings strip runs beside the salt table for a valid desktop blend.
