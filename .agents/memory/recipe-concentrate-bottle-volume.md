...
description: Recipe concentrate bottles scale salt mass by strength and dose volume by bottle size.
---
...
**How to apply:** Compute hydrated salt mass from the one-liter recipe target and strength. Compute dose as `bottleVolumeMl / strength` per liter, and pass the same bottle volume into per-mL, per-drop, and inverse pinned-strength calculations.