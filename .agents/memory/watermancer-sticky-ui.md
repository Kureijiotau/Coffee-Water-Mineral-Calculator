...
**How to apply:** Keep the full Final ion readings card in normal document flow unless the user enables Follow mode, which restores the fixed card with left/center/right dock choices and a compact-on-scroll summary. Reuse its exact row renderer for the tray, render the tray through a portal into `document.body` so card-level blur/overflow styles cannot contain it, size it to the calculator card’s max width, and use one full-width compact row per affected ion so labels and ppm values remain readable. Feedback is static with no animation; positive salt-dose changes and positive water-volume changes should open it. Do not make Follow and feedback mutually exclusive.