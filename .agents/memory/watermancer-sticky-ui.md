...
**How to apply:** Preserve the follow/docked toggle state as UI-only; if the result card layout changes, keep follow mode viewport-anchored rather than reintroducing ancestor-dependent sticky positioning. When follow mode exposes left/right dock controls, retain the selected dock across follow toggles. Keep the centered follow card height-limited on phones and let its ion rows scroll internally so it remains useful while editing the underlying recipe. When compact-on-scroll is enabled, hide the detailed ion rows only during active scrolling and restore the full card after scrolling settles; keep the GH/KH/GH:KH/TDS summary visible in the compact state.